	...
	if (this->nodeExists(node, "MvpExp")) {
		...
		mob->mexp = static_cast<t_exp>(cap_value((double)exp * (double)battle_config.mvp_exp_rate / 100., 0, MAX_EXP));
			...
			if (!this->asUInt64(node, "JobExp", exp))
				...
				mob->job_exp = static_cast<t_exp>(cap_value(((double)exp * (double)battle_config.job_exp_rate*battle_config.mvp_exp_rate/ battle_config.job_exp_rate), 0, MAX_EXP));

			if (!this->asUInt64(node, "BaseExp", exp))
				return 0;
					
				mob->base_exp = static_cast<t_exp>(cap_value(((double)exp * (double)battle_config.base_exp_rate*battle_config.mvp_exp_rate/ battle_config.base_exp_rate), 0, MAX_EXP));
